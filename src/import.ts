...
export async function importHistory(file: string, opts: ImportOptions): Promise<void> {
  if (!isGitRepo()) {
  ...
  const patchedStream = patchFastExportStream(stream, data.commits as Parameters<typeof patchFastExportStream>[1]);
